...
def get_start_url():
    with open('/home/zhang/python/work/91good_spider/file/shcool.txt') as f:
...
class GoodSchoolSpider(scrapy.Spider):
    ...
    # start_urls = ['https://www.91goodschool.com/school/3642.html']
    n = 1
    ...
    def lesson_url(self, response):
        ...
        next_page = response.xpath('//div[@class="page"]/a[last()-1]/@href').extract()
        if next_page:
            if next_page[0] == "javascript:void(0)":
                print('木有下一页了!')
            else:
                print('发现下一页课程')
                next_url = 'https://www.91goodschool.com' + next_page[0]
                yield scrapy.Request(next_url, callback=self.lesson_url, meta={'origin_url': response.meta['origin_url']})
    ...
    def lesson(self, response):
        ...
        logo_urls = response.xpath('//div[@class="pic"]/a/img/@src').extract()
        logo_urls = list(map(lambda url: 'https:' + url, logo_urls))
